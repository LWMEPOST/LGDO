...
1. P0：移除测试集特化 intent helper 与 200/300 分硬 boost，保留低权重通用检索信号。
2. P0：用 `entity_aliases`、默认别名 seed、alias context scoring 支撑角色/审批类别名召回。
3. P1：修复 Q20/Q21 链路，Q20/Q21 已可用内置 CLI 单题验证并通过。
4. P1：评测模块内置化，支持 GBrain OFF/ON/BOTH 对比、quality gate、failure diff、单题筛选和逐题进度。
5. P1：合并期间补齐 SQLite/PostgreSQL schema、迁移、旧索引和旧空 domain 兼容。
6. P2：BGE-M3 已作为可选 embedding provider 接入，默认仍保持 `local-hash-v1`，避免强依赖大模型包。

### 后续优化计划

| 优先级 | 任务 | 验收方式 |
|--------|------|----------|
| P0 | 把默认实体别名迁移到 YAML/JSON seed 文件 | 支持无需改 Python 即可新增角色、系统、文档别名；seed 幂等测试通过 |
| P1 | GBrain failure diff 增加失败标签 | 报告区分候选噪声、无命中、低质命中、LLM context 干扰、评分误判 |
| P1 | 增加单题超时与 GBrain 熔断 | both 模式不被单个慢题拖住；超时信息进入报告和 `retrieval_strategy` |
| P2 | retrieval features 配置化 | `contextual_boost()` 通用信号迁移到配置，报告输出 feature 命中原因和版本 |
| P2 | BGE-M3 高维 pgvector 索引迁移 | 按 embedding dimension 建表/索引，BGE-M3 不再只能 JSONB fallback |
| P3 | LLM streaming 和候选裁剪指标化 | P95 目标从当前秒级/十秒级向 `<15s` 收敛，报告展示 RAG/GBrain/LLM 分段耗时 |

### 执行原则

1. 不再把单题题面短语写进 reranker；新增召回能力优先走数据、配置或可观测 feature。
2. 每次检索调优必须跑 `pytest`、硬编码扫描和至少关键题 CLI 冒烟。
3. GBrain 继续作为可开关增强项，默认结论由 OFF/ON 对比报告决定。
4. 数据库 schema 变更后补迁移测试和旧数据兼容测试。
...
LGDO 已将 GBrain 从按请求启动的本地 stdio 子进程改为 `127.0.0.1:8787` 常驻 HTTP MCP 服务，并增加独立阿里云百炼 `text-embedding-v3` 语义重排通道。主链为 GBrain 图谱检索；GBrain 失败、超时、空结果或 ACL 后无结果时启用阿里语义重排；阿里云失败时保留 BM25/关键词结果。

## 技术选型

| 类别 | 选择 | 理由 |
|------|------|------|
| GBrain 通信 | 本机 HTTP MCP + Bearer token | 消除 Bun/PGLite 冷启动，保持鉴权与进程隔离 |
| 语义兜底 | 阿里云百炼 text-embedding-v3 1024 维 | 已有有效专属 API，中文语义能力优于本地哈希向量 |
| 最终兜底 | BM25/关键词 | 无外部依赖，保证服务可用性 |
| 进程管理 | PowerShell 幂等启动脚本 | 符合当前 Windows 开发部署环境 |

## 架构设计

- `scripts/start-lgdo.ps1` 检测健康状态，未启动时后台拉起 GBrain，LGDO 退出后 GBrain 保持运行。
- GBrain 仅绑定回环地址，LGDO 使用专属 Bearer token 请求 `/mcp`。
- 问答阶段并行执行本地候选召回与 GBrain 检索。
- GBrain 无授权结果时，阿里云对已召回候选进行 query/document 语义重排。
- 外部服务全部失败时不抛弃本地候选。

## 缓存与熔断

- GBrain 查询缓存 TTL 300 秒。
- 阿里 query embedding 缓存 TTL 600 秒，最多 256 条。
- GBrain 连续失败 3 次后熔断 30 秒，成功后自动复位。
- 百炼批次强制不超过 10 条，返回维度和条数严格校验。

## 实现与验证

- 新增外部 embedding provider、GBrain 诊断/熔断和问答降级融合。
- 新增 start/status/stop 三个 GBrain 运维脚本。
- GBrain PGLite 以 1024 维 schema 重建并导入 97 页、144 chunks。
- 后端测试：90 passed。
- 前端生产构建：通过。
- 实测 GBrain 返回 3 条结果，缓存命中；阿里返回 1024 维向量，缓存命中延迟为 0ms。

---

## 2026-07-13 15:03

# LGDO Revision、GBrain 与 Obsidian 闭环技术方案

## 技术方案概述

`raw/` 作为不可变证据，`vault/wiki` 作为人工可编辑内容事实源，revision 表保存完整历史；数据库检索、RAG 与 GBrain 都是可重建投影。重编译仅在当前版本仍等于最近生成版本时自动推进，否则保留人工正文并生成冲突候选。Vault watcher 把 Obsidian 文件事件转换为相同 revision 命令，所有下游更新通过持久化作业执行。

## 技术选型

| 类别 | 选择 | 理由 |
| --- | --- | --- |
| 修订历史 | SQLite/PostgreSQL 不可变 revision 表 | 支持比较、回滚、冲突候选和审计 |
| 内容事实源 | `vault/wiki` | 保留 Obsidian 的直接人工编辑体验 |
| 投影一致性 | 数据库 outbox + lease worker | API 不等待外部系统，失败可重放 |
| GBrain 写入 | 常驻进程内受控 MCP sync tool | 复用 PGLite engine，避免第二进程锁冲突和请求阻塞 |
| Vault 监听 | `watchfiles.awatch` | 支持 asyncio、Windows 与事件去抖 |
| Frontmatter | 受限 `ruamel.yaml` round-trip loader | 结构化校验并保留注释、键顺序和用户格式 |

## 架构设计

1. 编译、Web 编辑和 Vault watcher 统一调用 `WikiRevisionService`。
2. revision 通过持久 write intent、capture backup 和 no-replace 安装写入 Vault，任何未知并发内容都保留并进入冲突。
3. 人工版本与生成版本分设指针，重编译检测到分叉时创建冲突，绝不覆盖。
4. RAG 维护 current Wiki revision 投影；GBrain worker 合并作业后调用常驻 PGLite 进程内 MCP sync tool。
5. GBrain hit 必须映射到 current page/revision、active source 和授权用户后才能进入上下文并形成 Citation。
6. watcher 通过 page ID、revision、write token 和 hash 抑制应用自写回环，并在启动时对账停机期间的编辑。

## 模块与规格

- [Wiki Revision 与冲突模型](superpowers/specs/2026-07-13-wiki-revision-conflict-design.md)
- [GBrain 非阻塞导入与 Citation 契约](superpowers/specs/2026-07-13-gbrain-import-citation-design.md)
- [Vault Watcher 与 Obsidian 工作流](superpowers/specs/2026-07-13-vault-obsidian-workflow-design.md)

## 实施顺序

1. 幂等数据库迁移、revision 服务、冲突 API 与投影 outbox。
2. RAG Wiki 投影、GBrain 后台 worker 与 Citation 强契约。
3. Vault watcher、启动对账、稳定 `.obsidian` 配置与 Web deep link。
4. 全量单元、集成、重启恢复和运行态验收。

## 验收门槛

- 任何重编译都不会静默覆盖人工或 Obsidian 内容。
- `require_citations=true` 时所有事实答案至少包含一个授权且可定位的 Citation；无有效引用时返回明确拒答且不调用 LLM。
- compile/save 请求不等待 GBrain 同步，默认测试不访问真实外部服务。
- Obsidian add/modify/rename/delete 在 5 秒内进入 revision 与本地投影链路；GBrain 按异步 SLA 单独验收。
- watcher restart/replay 不产生重复 revision，RAG/GBrain 故障可恢复重放。
- 状态 API 能区分 watcher、RAG 和 GBrain 的 configured、running、clean、degraded。

三份书面规格已完成并发与端到端集成双重复审，未发现剩余 P0/P1；方案经确认，进入 TDD 实施计划。
