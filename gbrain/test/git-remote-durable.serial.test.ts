...
describe('pushProbe', () => {
  ...
  test('redactDetail scrubs a token from the failure detail', () => {
    const { work } = makePair();
    const token = `ghp_${'SECRETTOKEN'}`;
    git(work, 'remote', 'set-url', 'origin', join(root, `nope-${token}.git`));
    const r = pushProbe(work, 'main', { redactDetail: (s) => s.replaceAll(token, '***') });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.detail.includes(token)).toBe(false);
  });
});
