...
describe('buildGatewayConfig env-baseURL passthrough', () => {
  ...
  test('base_urls alias from gateway-shaped config also overrides env', async () => {
    await withEnv(
      { ...envFor(null), DASHSCOPE_BASE_URL: 'http://env.example/v1' },
      async () => {
        const cfg = buildGatewayConfig({
          base_urls: { dashscope: 'http://config.example/v1' },
        } as unknown as GBrainConfig);
        expect(cfg.base_urls?.dashscope).toBe('http://config.example/v1');
      },
    );
  });
});
