...
def test_list_entity_aliases_includes_legacy_empty_domain_alias(tmp_path, monkeypatch):
    from app.aliases import list_entity_aliases
    from app.db import connect, init_app_db, json_dump
    ...
    aliases = list_entity_aliases(settings, "product")

    assert any(item["id"] == "alias_legacy_global" for item in aliases)


def test_main_tables_are_unique_for_migration():
    from app.db import MAIN_TABLES

    assert len(MAIN_TABLES) == len(set(MAIN_TABLES))
