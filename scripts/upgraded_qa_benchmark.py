...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default=None, help="Directory for report.json/report.md/qa_results.csv")
    parser.add_argument("--mode", choices=["off", "on", "both"], default="both", help="Compare GBrain off/on modes")
    parser.add_argument("--domain", default=None, choices=["product", "customer_service", "administration"])
    ...
    parser.add_argument("--question-id", action="append", default=None, help="Question id to include, repeatable")
    parser.add_argument("--limit", type=int, default=None, help="Limit selected questions after filters")
    parser.add_argument(
        "--require-upgraded-data",
        action="store_true",
        help="Fail early when the local RAG store does not look like the full upgraded benchmark dataset.",
    )
...
def main() -> int:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else OUTPUT_ROOT / f"upgraded-qa-{started_at}"
    output_dir.mkdir(parents=True, exist_ok=True)

    settings = get_settings()
    status = rag_status(settings)
    if args.require_upgraded_data and status.get("source_count", 0) < 300:
...
def _settings_summary(settings: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "rag_embedding_provider": getattr(settings, "rag_embedding_provider", None),
        "rag_embedding_model": getattr(settings, "rag_embedding_model", None),
        "rag_embedding_dimension": getattr(settings, "rag_embedding_dimension", None),
    }
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        f"- RAG backend: `{report['settings'].get('rag_store_backend')}`",
        f"- GBrain enabled: `{report['gbrain_status'].get('enabled')}`",
        "",
        "## Summary",
        "",
